...
# File-scope `export _ll_shared:z/...` label definitions are split OUT of the
# public headers into companion .c files here, so a user TU that #includes a
# binding header does NOT re-emit those labels (they already live, once, in the
# LTO-built _ll_libmc.mch that every mcasm program includes). Compiled once into
# the stdlib bundle via build_libmc.sh's `find libmc -name "*.c"`.
...
def render_command_helpers_c(command: dict, asm_block: str) -> str:
    """Companion .c holding the file-scope `export _ll_shared:z/...` label
    definitions split out of the public header. Compiled once into the stdlib
    bundle; the header only carries the `static inline` wrappers that *reference*
    these labels (resolved at link time against _ll_libmc.mch)."""
    return f"""{GENERATED_NOTICE}

/*
 * File-scope command-helper label definitions for {command["name"]!r}, split out
 * of bindings/{command["header"]} so that a user translation unit which
...
def render_header(command: dict, commands_by_name: dict) -> tuple[str, str | None]:
    alias_of = command.get("alias_of")
    if alias_of is not None:
        return render_alias_header(command, commands_by_name[alias_of]), None

    if "variants" not in command:
        return render_empty_shell(command), None
    ...
    body, asm_block = render_structured_body(command)
...
def main() -> None:
    ...
    BINDINGS_SRC_DIR.mkdir(parents=True, exist_ok=True)
    for command in schema["commands"]:
        # hand_written commands own their bindings/<header> file (and any
        # companion .c) directly; the generator must not touch them (else the
        # empty-shell / stale body would clobber the maintained source).
        # vanilla.h still #includes the header below.
        if command.get("hand_written"):
            continue
        header, asm_block = render_header(command, commands_by_name)
        write_if_changed(BINDINGS_DIR / command["header"], header)
        # Companion .c with the file-scope `export` label defs (if any). Named
        # after the header stem so it stays discoverable and its lifecycle is
        # tied to this generated command only.
        c_path = BINDINGS_SRC_DIR / f"{Path(command['header']).stem}.c"
        if asm_block is not None:
            write_if_changed(c_path, render_command_helpers_c(command, asm_block))
        elif c_path.exists():
            # A previously-exporting command that no longer emits labels: drop
            # the now-stale companion so it can't shadow/duplicate anything.
            c_path.unlink()
