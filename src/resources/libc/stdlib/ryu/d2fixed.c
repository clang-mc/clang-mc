...
#include "d2fixed_dyadic.h"

#define DOUBLE_MANTISSA_BITS 52
#define DOUBLE_EXPONENT_BITS 11
#define DOUBLE_BIAS 1023

#define POW10_ADDITIONAL_BITS 128
...
static inline __attribute__((unused)) uint32_t mulShift_mod1e9(const uint64_t m, const uint64_t* const mul, const int32_t j) {
...
static inline uint32_t __attribute__((unused)) mulShift_mod1e9(const uint64_t m, const uint64_t* const mul, const int32_t j) {
  uint64_t high0;                                   // 64
...
static inline uint32_t __attribute__((unused)) indexForExponent(const uint32_t e) {
  return (e + 15) / 16;
}

static inline uint32_t __attribute__((unused)) pow10BitsForIndex(const uint32_t idx) {
  return 16 * idx + POW10_ADDITIONAL_BITS;
}

static inline uint32_t __attribute__((unused)) lengthForIndex(const uint32_t idx) {
  // +1 for ceil, +16 for mantissa, +8 to round up when dividing by 9
  return (log10Pow2(16 * (int32_t) idx) + 1 + 16 + 8) / 9;
}

static inline uint32_t div_ceil_u32(const uint32_t num, const uint32_t denom) {
  return (num + denom - 1) / denom;
}

static inline uint32_t ceil_log10_pow2_u32(const uint32_t e) {
  return log10Pow2((int32_t)e) + 1;
}

static inline uint32_t lengthForExponentNoTable(const uint32_t exponent) {
  return div_ceil_u32(ceil_log10_pow2_u32(exponent) + ceil_log10_pow2_u32(53), 9);
}

static inline uint32_t zeroBlocksAfterPointNoTable(const int32_t exponent) {
  if (exponent < -52) {
    const int32_t pos_exp = -exponent - 1;
    int32_t pos_len =
        ((int32_t)ceil_log10_pow2_u32((uint32_t)pos_exp) -
         (int32_t)ceil_log10_pow2_u32(53)) /
            9 -
        1;
    return pos_len > 0 ? (uint32_t)pos_len : 0;
  }
  return 0;
}

static inline bool isLowestBlockNoTable(const int32_t exponent,
                                        const uint32_t negative_block_index) {
  const int num_requested_digits = (int)((negative_block_index + 1) * 9);
...
int d2fixed_buffered_n(double d, uint32_t precision, char* result) {
  ...
  if (e2 >= -52) {
    const uint32_t pos_exp = e2 < 0 ? 0u : (uint32_t)e2;
    const uint32_t p10bits = POW10_ADDITIONAL_BITS + pos_exp;
    const int32_t len = (int32_t)lengthForExponentNoTable(pos_exp);
#ifdef RYU_DEBUG
    printf("idx=%u\n", pos_exp);
    printf("len=%d\n", len);
#endif
    for (int32_t i = len - 1; i >= 0; --i) {
      ...
      uint64_t mul[3];
      ryu_fill_pow10_split_exp(pos_exp, (uint32_t)i, mul);
      const uint32_t digits = ryu_mul_shift_mod1e9_exact(m2, mul, j);
      if (nonzero) {
...
#endif
  if (e2 < 0) {
    const uint32_t zero_blocks = zeroBlocksAfterPointNoTable(e2);
#ifdef RYU_DEBUG
    printf("zero_blocks=%u\n", zero_blocks);
#endif
    const uint32_t blocks = precision / 9 + 1;
    // 0 = don't round up; 1 = round up unconditionally; 2 = round up if odd.
    int roundUp = 0;
    uint32_t i = 0;
    if (blocks <= zero_blocks) {
      i = blocks;
      memset(result + index, '0', precision);
      index += precision;
    } else if (i < zero_blocks) {
      i = zero_blocks;
      memset(result + index, '0', 9 * i);
      index += 9 * i;
    }
    for (; i < blocks; ++i) {
      const uint32_t j = D2FIXED_CALC_SHIFT_CONST;
      uint64_t mul[3];
      if (isLowestBlockNoTable(e2, i) && i + 1 < blocks) {
      ...
      ryu_fill_pow10_split_2_exp((uint32_t)(-e2), i + 1, mul);
      uint32_t digits = ryu_mul_shift_mod1e9_exact(m2, mul, j);
#ifdef RYU_DEBUG
      printf("digits=%u\n", digits);
#endif
      if (i < blocks - 1) {
...
int d2exp_buffered_n(double d, uint32_t precision, char* result) {
  ...
  if (e2 >= -52) {
    ...
    for (int32_t i = len - 1; i >= 0; --i) {
      ...
      uint64_t mul[3];
      ryu_fill_pow10_split_exp(pos_exp, (uint32_t)i, mul);
      digits = ryu_mul_shift_mod1e9_exact(m2, mul, j);
      if (printedDigits != 0) {
  ...
  if (e2 < 0 && availableDigits == 0) {
    const uint32_t zero_blocks = zeroBlocksAfterPointNoTable(e2);
#ifdef RYU_DEBUG
    printf("zero_blocks=%u, e2=%d\n", zero_blocks, e2);
#endif
    for (uint32_t i = zero_blocks; i < 200; ++i) {
      const uint32_t j = D2FIXED_CALC_SHIFT_CONST;
      uint64_t mul[3] = {0, 0, 0};
      if (i == 0 || !isLowestBlockNoTable(e2, i - 1)) {
        ryu_fill_pow10_split_2_exp((uint32_t)(-e2), i + 1, mul);
        digits = ryu_mul_shift_mod1e9_exact(m2, mul, j);
      } else {
        digits = 0;
